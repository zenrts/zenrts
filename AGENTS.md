...
cmake --build build -j$(nproc)
ctest --test-dir build --output-on-failure
lcov --capture -d build --output-file coverage.info --branch-coverage --ignore-errors unused,version,empty,inconsistent
lcov --remove coverage.info -o coverage.info '/usr/*' '*/_deps/*' '*/tests/*' '*/benchmarks/*' '*/googletest/*' '*/googlebenchmark/*' --ignore-errors unused,empty,inconsistent
